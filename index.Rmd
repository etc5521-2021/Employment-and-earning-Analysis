...
Before fitting a model to the data, an important factor to be considered is that **the earning data is time series data**, median weekly incomes naturally grow across all variables of interests over the years, assumption of independence and randomness is violated in this case. The best possible solution under this circumstances is to consider year as an additional categorical variable and include it in the model.
...
```{r}
resid_panel(model, plots = "default")
```
```{r fit}
glance(model)%>% 
